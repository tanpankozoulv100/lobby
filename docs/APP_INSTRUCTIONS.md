...
| 層 | 正本 | 内容 |
|----|------|------|
| **登録データ** | `events/{id}/slotChoices` | シーズン分の枠（スプレッドシート → Send Pending Rows）。シーズン開始前にほぼ入力済み想定（後から追加も可） |
| **公開制御** | `eventDisplayWindow/current` + `users/{uid}/cohortWeeks/{weekKey}` | ユーザーに見せる1週間と、その週の A/B |

### 4.2 ユーザーに見える範囲

- **常に1週間のみ**: 次週の **日曜〜土曜**（`visibleFromDateKey`〜`visibleToDateKey`）。
- **毎週木曜 12:00 JST 頃**（管理側 Cron）で次週分に切り替わる。ユーザーアプリは結果を読むだけ。
- 表示週未設定時は枠を表示しない（準備中メッセージ）。

### 4.3 A/B コホート（週次・不変）

- 割当: `users/{uid}/cohortWeeks/{weekKey}`（木曜の週次処理と同タイミング）。
- **過去週は書き換えない**: 先週 A → 今週 B になっても、先週の `cohortWeeks` レコードは A のまま。日付ごとにその週の割当を参照する。
- 枠の `cohort`（A/B）とユーザー割当が一致する枠だけ表示。
- UI: 表示週バナー + 「毎週木曜に次週分へ更新」（`dashboard-events-section.tsx`）。

**運用の自動化（アプリ外）:** 詳細は `lobby-admin/docs/ADMIN_INSTRUCTIONS.md` §3.1。
...
| 2026-06-06 | **ロビー空間UI（初版）**: 世界観を「現実の隣のパラレルワールドのロビー」に再定義。ダッシュボードを**円形の壁が5面並ぶ3D空間**（`LobbyRoom`）に変更—ボトムナビでメニューを切替えると視点がぐるっと回り、履歴→イベント→ホーム→レター→マイページの壁を向く。暗い**ボルドー色の洋風ロビー**（木製パネル壁・シャンデリアの暖色・床の反射・参考画像の環境光）。各壁のニッチ（展示窓）内に従来の画面コンテンツを表示。左右スワイプでも隣の壁へ回転可能 |
| 2026-06-06 | **イベント週次表示:** スプシはシーズン分を事前登録、ユーザーには毎週木曜に公開される次週（日〜土）1週間のみ表示。`cohortWeeks` は週ごと固定（過去週は書き換えない）。表示週未設定時は枠非表示 |
