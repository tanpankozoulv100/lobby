...
### A. 教えてほしい（現状把握）
- [ ] **いま手元で管理画面（やアプリ本体）を別のコピー／バックアップ（別フォルダ・別リポジトリ・別ブランチ・ローカルのみ等）で触っていませんか？**
  もしあれば、その変更はこの共有リポジトリ `dino0402r/Lobby` の `main` に **push して反映** してください（→ 後述「7. コードの共有・同期ルール」）。
  共有リポジトリに入っていない変更があると、こちらの修正と食い違い／上書きの原因になります。
...
現在、現・開発担当（小島）と過去の運営者の**双方が同じアプリを触っている**状態です。
変更が片方の手元だけ（別コピー／バックアップ／ローカル）に留まると、もう片方の修正と
**食い違い・上書き・デプロイ漏れ**が発生します。これを防ぐため、以下を必ず守ってください。

### 唯一の正（Single Source of Truth）
- 正本のコードは **GitHub: `dino0402r/Lobby` の `main` ブランチ** です。
- 本番（Vercel）も `main` から配信されます。
- **ローカルや別フォルダだけで作業して終わりにしない**でください。必ず下記の手順で `main` に反映してください。

### 作業前（毎回）
```bash
git checkout main
git pull origin main      # 最新を取り込んでから作業開始（食い違い防止）
```

### 作業後（毎回）
```bash
git add -A
git commit -m "何を変えたか簡潔に"
git pull --rebase origin main   # 相手の更新を取り込む
git push origin main
```
- コンフリクト（衝突）が出たら、勝手に握りつぶさず、どちらの変更を残すか小島と相談してください。
- 大きめの変更は可能なら **ブランチを切って Pull Request** にしてもらえると、こちらで確認してから取り込めます。

### もし既に「別コピー／バックアップ（BU）」で進めてしまっている場合
1. その差分を失わないよう、まず現物（フォルダ/zip 等）を保全。
2. `dino0402r/Lobby` をクローン or 既存ローカルで `git pull` し、**変更点だけを移植**して commit → push。
3. どのファイルを触ったか分からない場合は、その別コピー一式を小島に共有してください。こちらで差分を取り込みます。

### 反映されたことの確認
- push 後、GitHub の `main` に自分のコミットが見えること。
- 本番URL（https://lobby-mu-six.vercel.app）に反映されること（デプロイは小島側でも実行可能）。

> まとめ: **「手元だけ」で終わらせず、必ず `main` に push。BU（別コピー）があるなら共有・反映してください。**
