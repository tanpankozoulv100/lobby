...
**シーズン・シリアル・参加人数・No. 採番**は `docs/SEASON_AND_TICKET_SPEC.md` を優先します。
...
**チケット照合:** `ticketCodes/{code}` に `intendedGender`（`male` / `female`）を運営が付与。引き換え時にユーザーの `gender` と一致しない場合は拒否（男性用・女性用で Shopify 価格が異なるため）。**シーズン紐づけ:** チケットに `seasonId`（`seasons/{id}`）を付与すると、引き換え時に `users/{uid}.currentSeasonId` が設定される。同時期の複数開催・同年の同一開催地複数回に対応。表示文言・終了日・参加人数・A/B コホートキーは `seasons` を正本（管理サイト `/dashboard/seasons`）。`seasonId` 未設定の旧チケットは `isLegacyDefault` の published シーズンへフォールバック。
...
- 参加者番号 **No.001〜** は、**シーズンチケット（シリアル）引き換え順** でシーズンごとに付与（`system/seasonParticipantCounters/{seasonId}`）。**001–999 は3桁表示、1000以降は4桁表示**。運営（`admins`）は **No.000** 固定（ダッシュボード初回開放時）。
- 参加人数表示は `seasons.redeemedCount`（シリアル引き換え時に自動加算）。シリアルは管理サイトで `{場所}{年}{回数}{x|y}{ランダム}` 形式で発行。
- 参加者は A/B コホートで分岐。
...
- **週次 A/B + 表示週:** `lobby-admin` の `GET /api/cron/weekly-operations`（木曜 JST）。スプシ登録済み枠のうち **次週（日〜土）1週間だけ**を `eventDisplayWindow/current` で公開。`cohortWeeks/{weekKey}` は週ごとに固定（過去週は不変）。手動は緊急時のみ。§ `lobby-admin/docs/ADMIN_INSTRUCTIONS.md` §3.1。
